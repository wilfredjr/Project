     ...
     <!--        <li class="<?php echo (substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1))=="time_management.php"?"active":"";?>">
      ...
      -->       
            ...
            <?php
             ...
             $tasker=$con->myQuery("SELECT id FROM projects_employees WHERE (SELECT is_team_lead_ba=1 OR is_team_lead_dev=1 OR is_manager=1 WHERE employee_id=:employee_id)",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchAll();
             if(!empty($tasker)){
             ?>
            ...
            <?php }
            $bugs=$con->myQuery("SELECT count(id) FROM project_bug_list WHERE (manager_id=:employee_id OR team_lead_ba=:employee_id OR team_lead_dev=:employee_id OR ba_test=:employee_id OR dev_control=:employee_id) AND project_status_id!=2",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchColumn();
            $bugs=intval($bugs);
            if(!empty($bugs)){  ?>
            <li class="<?php echo (substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1))=="bug_management.php"?"active":"";?>">
              <a href="bug_management.php">
...
<!--FOR APPROVAL MENU-->
               ...
               <?php
                 ...
                 $project_phase_approval_count=$con->myQuery("SELECT COUNT(ppr.id) FROM project_phase_request ppr WHERE ppr.request_status_id=1 AND (SELECT   
                  ...
                  $project_dev_approval_count=$con->myQuery("SELECT COUNT(pd.id) FROM project_development pd WHERE pd.request_status_id=1 AND (SELECT   
                         ...
                         END=:employee_id)",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchColumn();
                 $task_management_approval_count=$con->myQuery("SELECT COUNT(id) FROM project_task WHERE request_status_id=1 AND manager_id=:employee_id",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchColumn();
                 $task_completion_approval_count=$con->myQuery("SELECT COUNT(id) FROM project_task_completion WHERE request_status_id=1 AND (manager_id=:employee_id OR team_lead_id=:employee_id)",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchColumn();
                 if($usertype=='4'){
                 $project_application_approval_count=$con->myQuery("SELECT COUNT(id) FROM project_application WHERE request_status_id=1",array("employee_id"=>$_SESSION[WEBAPP]['user']['employee_id']))->fetchColumn();
                 }else{$project_application_approval_count="";}

                  $total_for_approval=intval($ot_count) + intval($leave_count) + intval($ob_count) + intval($shift_count) + intval($adj_count) + intval($offset_count)+intval($allowance_count)+intval($ot_adjustment_count) + intval($project_employee_approval_count) + intval($project_phase_approval_count) + intval($task_management_approval_count) + intval($task_completion_approval_count)  + intval($project_application_approval_count + intval($project_dev_approval_count));
              ...
              <li class='treeview <?php echo (in_array(substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1), array("overtime_approval.php","leave_approval.php","ob_approval.php","shift_approval.php","adjustments_approval.php","offset_approval.php", "allowance_approval.php","ot_adjustments_approval.php","project_employee_approval.php","project_phase_approval.php","task_management_approval.php","task_completion_approval.php","project_application_approval.php","project_development_approval")))?"active":"";?>'>
                  ...
                  <ul class='treeview-menu'>                
                      ...
                      <?php } if(!empty($project_dev_approval_count)){ ?>
...
<!--EMPLOYEE SELF SERVICE MENU-->
           <?php
            ...
            $total_return=intval($ot_return_count) + intval($leave_query) + intval($adj_query) + intval($shift_query) + intval($ob_query)+intval($allowance_query)+intval($project_employee_query)+intval($project_phase_query)+intval($task_management_query)+intval($project_development_query);
            ...
            <li class='treeview <?php echo (in_array(substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1), array("overtime.php","employee_leave_request.php","adjustment_request.php","shift_request.php","ob_request.php","offset.php", "allowance_request.php","project_employee_request.php","project_phase_request.php","task_management_request.php","project_development_request.php")))?"active":"";?>'>
                ...
                <ul class='treeview-menu'>
                   ...
                   <li class="<?php echo (substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1))=="overtime.php"?"active":"";?>">
                    ...
                    ?> -->
                    <?php if(($usertype!='5')OR($usertype!='2')){?>
                    ...
                    <?php }else{ ?>
                    <li class="<?php echo (substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1))=="bug_management_request.php"?"active":"";?>">
                      <a href="bug_management_request.php"><i class="fa fa-circle-o"></i> <span>Bug Assignment</span> <?php echo empty($project_employee_query)?'':"<small class='label pull-right bg-primary'>{$project_employee_query}</small>";?></a>
                    </li>
                    <?php } ?>
                </ul>
            </li>
             <li class='treeview <?php echo (in_array(substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1), array("my_project_files.php","my_files.php")))?"active":"";?>'>
              <a href="#">
...
<!--END OF EMPLOYEE SELF SERVICE MENU-->
            ...
            <li class='treeview <?php echo (in_array(substr($_SERVER['PHP_SELF'],strrpos($_SERVER['PHP_SELF'], "/")+1), array("audit_log.php","employees.php","terminated_employees.php","users.php","education_level.php","skills.php","trainings.php","certifications.php","job_title.php","leave_type.php","departments.php","monitor_attendance.php","company_files.php","employee_files.php","tax_status.php","pay_grade.php","employment_status.php","approval_matrix.php","settings.php","company_settings.php")))?"active":"";?>'>
