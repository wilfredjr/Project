<?php
	...
	if(!empty($_GET['id']))
    {
  		$data=$con->myQuery("SELECT pbl.id,pbl.bug_phase_id,pbl.name as bug_name,pbl.employee_id,pbl.description,p.name AS project_name, pbl.project_id,pbl.project_status_id,pbp.name as phase_name, ps.status_name, pbl.date_filed,pbl.date_start,pbl.date_end,pbr.desc as bug_rate,pbl.manager_id,pbl.team_lead_ba,pbl.team_lead_dev,pbl.ba_test,pbl.dev_control,
...
JOIN project_bug_phase pbp ON pbl.bug_phase_id=pbp.id JOIN project_bug_rate pbr ON pbr.id=pbl.bug_rate_id WHERE (pbl.manager_id='$employee_id' OR pbl.team_lead_ba='$employee_id' OR pbl.team_lead_dev='$employee_id' OR pbl.ba_test='$employee_id' OR pbl.dev_control='$employee_id' OR pbl.employee_id='$employee_id') AND pbl.id=? LIMIT 1",array($_GET['id']))->fetch(PDO::FETCH_ASSOC);
  ...
  $dates=$con->myQuery("SELECT pbpd.date_start,pbpd.date_end,pbp.name FROM project_bug_phase_dates pbpd JOIN project_bug_phase pbp ON pbp.id=pbpd.bug_phase_id WHERE project_id=? AND bug_list_id=? AND bug_phase_id='1'",array($data['project_id'],$data['id']))->fetch(PDO::FETCH_ASSOC);
    ...
    $current=$con->myQuery("SELECT bf.id FROM  bug_files bf JOIN project_bug_list pbl ON pbl.id=bf.bug_list_id  WHERE bf.bug_list_id=? AND bf.is_deleted=0 AND pbl.project_id=? AND bf.employee_id=?",array($data['id'],$data['project_id'],$data['employee_id']))->fetch(PDO::FETCH_ASSOC);
                        ...
                        <div class="row">
                            <div class='col-md-12'>
                                <div class='col-md-9  col-md-offset-1' >
                                      ...
                                      <tr><td><b>Account Manager: </b><?php echo $data['account_manager'];?> </td><td><b>Account Manager File: </b><a href="download_file.php?id=<?php echo $current['id']."&type=bf";?>" class='btn btn-default'><span class='fa fa-download'></span></a> </td></tr>
                                    ...
                                    <div>
                                      <div class='form-group'>
                                            ...
                                            ?>
                                            <?php
                                            if(($data['bug_phase_id']=='2')AND($data['ba_test']==$employee_id)){
                                            echo "<div class='col-md-offset-3'>";
                                            echo "<button type='button' style='display: inline;' class='btn btn-warning' onclick='submit(\"{$data['id']}\")'>Endorse as Fixed</button>"; 
                                            echo "<button type='button' style='display: inline;' class='btn btn-danger' onclick='submit(\"{$data['id']}\")'>Endorse Errors</button>";}?>
                                        </div>
                                    </div>
                                <form class='form-horizontal disable-submit' action='save_bug_employee.php' method="POST">
                                    ...
                                    <div class='form-group'>
                                        <label for='dev' class='col-sm-2 control-label'>Developer: </label>
                                          <div class='col-sm-9'>
                                            <select class='form-control cbo' name='developer' data-allow-clear='True' data-placeholder='Select Developer (Bug Control)'  <?php echo !(empty($data))?"data-selected='".$data['dev_control']."'":NULL ?> required <?php if(($data['manager_id']==$employee_id)OR($data['team_lead_dev']==$employee_id)){}else{?> disabled <?php }?> >
                                    ...
                                    <div class='form-group'>
                                        <label for='dev' class='col-sm-2 control-label'>Tester: </label>
                                          <div class='col-sm-9'>
                                            <select class='form-control cbo' name='tester' data-allow-clear='True' data-placeholder='Select BA (Bug Assessment)'  <?php echo !(empty($data))?"data-selected='".$data['ba_test']."'":NULL ?> required<?php if(($data['manager_id']==$employee_id)OR($data['team_lead_ba']==$employee_id)){}else{?> disabled <?php }?> >
                                               <?php echo makeOptions($dev_control); ?>   
                                          </select>
                                        </div>
                                    </div>
                                    <?php if(($data['manager_id']==$employee_id)OR($data['team_lead_ba']==$employee_id)OR($data['team_lead_dev']==$employee_id)){?>
                                    <div class="form-group">
