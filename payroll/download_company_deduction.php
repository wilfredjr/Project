...
$objPHPExcel->getActiveSheet()->setTitle('Sheet1');
    ...
    $inputFileName = 'files/company_deduction.xlsx';
    ...
    while ($data=$data_query->fetch(PDO::FETCH_ASSOC)) 
    {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$row, $data['comde_desc']);
        $objPHPExcel->getActiveSheet()->getStyle('D'.$row)->getNumberFormat()->setFormatCode('##,##0.00');
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$row, $data['emp_comdeduc']);
        $objPHPExcel->getActiveSheet()->insertNewRowBefore($row + 1, 1);

        $total += $data['emp_comdeduc'];
    ...
    header('Content-Disposition: attachment;filename="company_deduction.xlsx"');
