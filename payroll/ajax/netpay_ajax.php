...
$columns = array(
    ...
    array( 'db' => 'tax_status','dt' => ++$index ,'formatter'=>function($d,$row){
     ...
     array( 'db' => 'name','dt' => ++$index ,'formatter'=>function($d,$row){
      ...
      array( 'db' => 'emp_netpay','dt' => ++$index ,'formatter'=>function($d,$row){
...
$query="SELECT e.code,p.tax_compensation AS emp_tax_comp,tax_status.code as 'tax_status',CONCAT(e.last_name,', ',e.first_name,' ',e.middle_name) as name, p.net_pay AS emp_netpay  FROM payroll_details p INNER JOIN employees e ON p.employee_id=e.id INNER JOIN tax_status ON p.tax_compensation = tax_status.id";
