...
$columns = array(
    array( 'db' => 'ph_code','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
     array( 'db' => 'ph_from_comp','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
      array( 'db' => 'ph_to_comp','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
       array( 'db' => 'ph_ee','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
        array( 'db' => 'ph_er','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
            ...
            $action_buttons.="<button type='button' data-toggle='modal' data-target='#myModal' data-p_code='{$row['ph_code']}'
                                        data-r_comp_from='{$row['ph_from_comp']}' data-r_comp_to='{$row['ph_to_comp']}' data-ee_share='{$row['ph_ee']}' data-er_share='{$row['ph_er']}'  class='btn btn-sm btn-danger' onclick='pass(this)''><span class='fa fa-edit'></span></button>";


            $action_buttons.=" <a href='delete.php?cd={$row['ph_code']}&tb=p' class='btn btn-sm btn-danger' onclick='return confirm(\"Are you sure you want to delete this entry?\")' ><span class='fa fa-trash'></span></button>";
...
$query="SELECT * FROM gd_philhealth";
...
$recordsTotal=$con->myQuery("SELECT COUNT(id) FROM gd_philhealth {$where};",$bindings)->fetchColumn();
