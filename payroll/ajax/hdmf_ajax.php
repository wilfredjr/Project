...
$columns = array(
    array( 'db' => 'hdmf_code','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
     array( 'db' => 'hdmf_from_comp','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
      array( 'db' => 'hdmf_to_comp','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
       array( 'db' => 'hdmf_ee','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
        array( 'db' => 'hdmf_er','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
         array( 'db' => 'hdmf_cont_option','dt' => ++$index ,'formatter'=>function($d,$row){
        return htmlspecialchars($d);
    }),
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
            ...
            $action_buttons.="<button type='button' data-toggle='modal' data-target='#myModal' data-h_code='{$row['hdmf_code']}'
                                    data-r_comp_from='{$row['hdmf_from_comp']}' data-r_comp_to='{$row['hdmf_to_comp']}'
                                    data-ee_share='{$row['hdmf_ee']}' data-er_share='{$row['hdmf_er']}' data-hdmf_cont_option='{$row['hdmf_cont_option']}' class='btn btn-sm btn-danger' onclick='pass(this)'><span class='fa fa-edit'></span></button>";


            $action_buttons.=" <a href='delete.php?cd={$row['hdmf_code']}&tb=h' class='btn btn-sm btn-danger' onclick='return confirm(\"Are you sure you want to delete this entry?\")' ><span class='fa fa-trash'></span></button>";
...
$recordsTotal=$con->myQuery("SELECT COUNT(id) FROM gd_hdmf {$where};",$bindings)->fetchColumn();
