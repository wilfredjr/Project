...
$columns = array(
    ...
    array( 'db' => 'id','dt' => ++$index ,'formatter'=>function ($d, $row) {
         if(!empty($_GET['id'])){
            ...
            $get_emp_id=$con->myQuery("SELECT employee_id FROM projects_employees WHERE project_id = ? AND is_deleted = 0 AND employee_id=?",array($_GET['id'],$d))->fetch(PDO::FETCH_ASSOC);
            if(!empty($get_emp_id)){
...
$whereAll=" es.name <> 'Resigned' and es.name <> 'Terminated' and e.is_deleted =0 and e.is_terminated =0";
$whereResult="";
