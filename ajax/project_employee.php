...
if(!empty($_GET['id'])){
    $whereAll=" es.name <> 'Resigned' AND es.name <> 'Terminated' AND e.is_deleted ='0' AND esd.is_deleted = '0' AND esd.project_id = ".$_GET['id'] ;
}else{
    $whereAll="";
}
$whereResult="";
...
$join_query=" INNER JOIN job_title jt ON e.job_title_id = jt.id INNER JOIN employment_status es ON e.employment_status_id = es.id INNER JOIN departments d ON e.department_id = d.id INNER JOIN projects_employees esd ON esd.employee_id = e.id ";

$bindings=jp_bind($bindings);
$complete_query="SELECT e.id, e.code as emp_code, CONCAT(e.first_name,' ',e.middle_name,' ',e.last_name) AS emp_name, CONCAT(d.name, ' (',d.description,')') as department, jt.description AS job_title, esd.is_deleted FROM employees e {$join_query} {$where} {$order} {$limit}";
