    ...
    array(
        ...
        'formatter' => function ($d, $row) {
            ...
            // $action_buttons.="<button type='button' data-toggle='modal' data-target='#myModal' class='btn btn-sm btn-danger'><span class='fa fa-search'></button>&nbsp;";
                 $action_buttons.="<a href='my_projects_view.php?id={$row['id']}' class='btn btn-sm btn-warning'><span class='fa fa-search'></span></a>&nbsp;";
...
$whereAll=" pe.employee_id=:employee_id AND p.is_deleted=0";
    $filter_sql="";
    $filter_sql.=" ";
...
FROM projects p 
JOIN projects_employees pe ON pe.project_id=p.id JOIN project_status ps ON p.project_status_id=ps.id {$where} {$order}";
            // echo $complete_query;
             //var_dump($bindings);
