...
$columns = array(
    ...
    array(
        ...
        'formatter' => function ($d, $row) {
            ...
            $action_buttons.="<a class='btn btn-sm btn-warning' href='frm_holiday.php?id={$row['id']}' title='Edit' ><span  class='fa fa-pencil'></span></a>";
            $action_buttons.="<form method='post' style='display:inline' action='delete_holidays.php' onsubmit='return confirm(\"Are you sure you want to delete this holiday?\")'><input type='hidden' name='id' value='{$row['id']}'> <button class='btn btn-sm btn-danger' type='submit' title='Delete'><span  class='fa fa-trash'></span></button></form>";
...
if (!empty($_GET['pay_group_id'])) {
    $sa_id_sql="h.payroll_group_id = :paygroup_id";
    
    if (!empty($filter_sql)) {
        $filter_sql.=" AND ";
    }
$bindings[]=array('key'=>'paygroup_id','val'=>$_GET['pay_group_id']."",'type'=>0);
...
$complete_query="SELECT h.id,h.holiday_name,h.holiday_date,h.holiday_day,h.holiday_category FROM holidays h {$join_query} {$where} {$order} {$limit}";
            // echo $complete_query;
             //var_dump($bindings);
...
$recordsTotal=$con->myQuery("SELECT COUNT(h.id) FROM holidays h {$join_query} {$where};", $bindings)->fetchColumn();
