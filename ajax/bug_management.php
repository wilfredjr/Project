    ...
    array(
        ...
        'formatter' => function ($d, $row) {
            ...
            // $action_buttons.="<button type='button' data-toggle='modal' data-target='#myModal' class='btn btn-sm btn-danger'><span class='fa fa-search'></button>&nbsp;";
                 $action_buttons.="<a href='bug_management_project.php?id={$row['id']}' class='btn btn-sm btn-warning'><span class='fa fa-search'></span></a>&nbsp;";
...
$whereAll="p.is_deleted=0 AND p.project_status_id=2";
    $filter_sql="";
    $filter_sql.=" ";
...
$complete_query="SELECT p.id,p.name,p.description,p.start_date,p.end_date,p.project_status_id,ps.status_name,
...
(SELECT CONCAT(e.last_name,', ',e.first_name) FROM employees e WHERE id=p.manager_id) AS manager_id
FROM projects p 
...
$recordsTotal=$con->myQuery("SELECT COUNT(p.id) FROM `projects` p {$where};",$bindings)->fetchColumn();
