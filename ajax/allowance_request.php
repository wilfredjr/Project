    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
            ...
            if(!empty($row['evidence']) && file_exists("../allowance_evidence/".$row['evidence'])){
                $action_buttons.="<button class='btn btn-sm btn-info ' onclick='show_image_modal(\"{$row['evidence']}\")' title='View Evidence'><span class='fa fa-
                search'></span></button>";
            }
            ...
            if($row['request_status_id']<>2 && $row['request_status_id']<>5 && $row['request_status_id']<>4 ):
                ...
                $action_buttons.=" <button class='btn btn-sm btn-danger' value='allowance' title='Cancel Request'><span class='fa fa-trash'></span></button></form>";
            endif;
...
$complete_query="SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSP::pluck($columns, 'db'))."`
             FROM `vw_employees_allowances` {$where} {$order} {$limit}";
            // echo $complete_query;
             //var_dump($bindings);
