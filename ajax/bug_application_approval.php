...
$columns = array(
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
            global $con;
            $action_buttons="";
            $current=$con->myQuery("SELECT id FROM  bug_files WHERE bug_list_id=? AND is_deleted=0",array($row['id']))->fetch(PDO::FETCH_ASSOC);
            if(!empty($current['id'])){
            ...
            $action_buttons.="<input type='hidden' name='type' value='bug_application_approval'>";
...
$whereResult="";
 ...
 $query="SELECT pa.id,pa.name,pa.description as des,pa.request_status_id,pa.employee_id,p.name as project,pa.date_filed,pa.reason,rs.name as request_name,pbr.desc as bug_rate,
...
(SELECT CONCAT(e.last_name,', ',e.first_name) FROM employees e WHERE e.id=pa.manager_id) AS manager
FROM project_bug_application pa";
$join=" JOIN request_status rs ON rs.id=pa.request_status_id JOIN project_bug_rate pbr ON pa.bug_rate_id=pbr.id
...
$recordsTotal=$con->myQuery("SELECT COUNT(pa.id) FROM `project_bug_application` pa {$join} {$where};",$bindings)->fetchColumn();
